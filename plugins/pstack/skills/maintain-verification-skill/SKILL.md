...
Find project skills under `.agents/skills/` for Codex, `.grok/skills/` for Grok Build. If the repository already uses a project skill directory recognized by the current host, include it in the search. Edit only the selected verification skill.
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `verify-*/` under the host's project skill directory). Several candidates → ask which one; none → stop and point at `create-verification-skill` instead of inventing a target.
