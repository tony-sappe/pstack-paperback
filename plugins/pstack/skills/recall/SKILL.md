...
Use only task or session history exposed by the current host, or history the user supplies. Do not infer private transcript paths or scan another project. If prior history is unavailable, reconstruct from the current task, repository, and shared records, and label that limit.
...
Use the task/session listing and reading tools the host actually exposes. A saved user-provided export is also valid. Never synthesize a filesystem path from the workspace name.

1. Classify, then route. One specific prior task to resume is the `session-pickup` playbook, not this. Turning habits into a durable skill belongs in the current host's skill-authoring flow (`skill-creator` in Codex, the documented `SKILL.md` format in Grok Build). A human-readable summary of your work is a different task. Recall loads working context across recent tasks before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
...
3. Search available task history for the topic, sort by the host's actual update time, and read only relevant turns. Skip the current task and obvious noise (subagent, eval, and test tasks). For a large accessible corpus, delegate independent slices when the host permits; for one or two tasks, read directly. Return one block per task: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), with the host's task identifier. If no prior history is available, say so and continue with the current task and shared record.
