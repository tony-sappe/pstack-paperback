...
Use only tools and delegation features available in the current host. Instructions elsewhere in this skill stack may use Cursor labels such as `Task`, `subagent_type`, Cursor model slugs, or `/loop`; translate those steps to the host's native capability. If the host has no equivalent, do the work sequentially when practical and state the limitation. Never claim a delegated run, model selection, or wake loop happened unless the host actually provided it.
