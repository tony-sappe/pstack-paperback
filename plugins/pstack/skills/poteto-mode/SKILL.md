...
Use the tools and delegation features available in Codex or Grok Build. Read [host compatibility](../../HOST-COMPATIBILITY.md) for supported package behavior. If a host capability is unavailable, do the work sequentially when practical and state the limitation. Never claim a delegated run, model selection, or scheduled continuation happened unless the host actually provided it.
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface. Write it per **Writing the reply**. For agent-facing prose, use the host's skill-authoring guidance when available.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → review the diff for dead paths, narration, and unrelated edits.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → drive the real surface with an available host tool or project harness. For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`). That includes "babysit this", "get it green", and "check on PR X". Never triggered by merely opening a PR. Declare its mode before polling.
- Asked to land or ship a green stack → verify each PR independently and follow the current host's supported forge and approval flow. Green is not a verification verdict.
- Review comments → verify each claim against the code. Fix real findings and explain dismissed ones with evidence.
...
**Act within authorization.** Reversible local work can proceed. Use external tools, write to services, contact people, or launch paid evaluations only when the user or host permissions authorize those actions. A workflow does not grant permission by itself.

**Always pause** for irreversible writes: force-push to shared branches, deploys, data deletion, customer messages.

**Session overrides:** "Don't stop" / "going to bed" / "run until done" / "be fully autonomous" → keep going within the user's authorization and the host's available wake mechanism.
...
For a playbook, open a task list with its applicable steps before task-specific items. Use the host's tools for each step. Record an unavailable step as `skip: <reason>` with its impact on the result.

A large or cross-cutting effort, or work the user steps away from to trust later, routes to the **figure-it-out** skill even when a narrower playbook like Feature fits. Use **figure-it-out** whenever no bundled playbook fits. It designs a bespoke playbook for the task.
