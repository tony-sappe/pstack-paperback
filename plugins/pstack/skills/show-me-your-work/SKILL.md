...
Transcript audit depends on a workspace-scoped transcript directory supplied by the host. If the host does not provide it, do not guess private transcript paths; report that transcript auditing is unavailable and continue only with evidence the user or current session supplied.
