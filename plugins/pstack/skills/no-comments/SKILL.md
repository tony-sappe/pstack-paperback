...
1. If the host can delegate, pass the scope and [Comment Sicko prompt](../../agents/comment-sicko.md) to a native subagent. Grok Build can select the bundled `pstack:comment-sicko` agent when available. If delegation is unavailable, apply that prompt yourself and identify the review as self-review.
